...
@Slf4j
public class OldDataListener extends AnalysisEventListener<OldData> {

    // 用于接收解析的所有数据
    private Map<Integer, ReadCellData<?>> headMaps = null;
    private List<OldData> allDatas = new ArrayList<>();

    // 提供一个对外访问的方法,返回数据
    public List<OldData> getDatas() {
        return allDatas;
    }
    ...
    @Override
    public void invoke(OldData data, AnalysisContext Context) {
        // 读取数据加入总数据
        allDatas.add(data);
        count++;
